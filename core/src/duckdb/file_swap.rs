...
//!    every *other* table, view, and index — other datasets sharing the file,
//!    `spice_sys_*` metadata tables — into it. The refreshed dataset's stale
...
pub(super) fn insert_overwrite_swap(
    ...
    let num_rows = stage_refreshed_data(
        &pool,
        table_definition,
        &building_path,
        batch_rx,
...
fn copy_live_contents_into_staging(
    ...
    let seq = GENERATION_SEQ.fetch_add(1, Ordering::Relaxed);
    let stage_alias = format!("__spice_swap_stage_{seq}");
...
mod tests {
    ...
    async fn test_overwrite_file_swap_end_to_end() {
        ...
        // metadata table, all of which the swap must carry over.
        {
            ...
            conn.execute_batch(
                "CREATE TABLE other_data (id BIGINT, tag VARCHAR);
                 ...
                 CREATE TABLE spice_sys_dataset_checkpoint (dataset_name TEXT PRIMARY KEY, created_at TIMESTAMP);
                 INSERT INTO spice_sys_dataset_checkpoint VALUES ('other', now());",
            )
        ...
        assert_eq!(
            count(
                &conn,
                "SELECT COUNT(1) FROM spice_sys_dataset_checkpoint WHERE dataset_name = 'other'"
            ),
            1
        );
        assert_eq!(
